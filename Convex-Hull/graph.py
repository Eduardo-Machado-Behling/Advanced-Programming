...
def DrawVoronoi(df: pd.DataFrame) -> None:
    fig, ax = plt.subplots(1,1, figsize=(18,20))

    def plot(ax: Axes, y: str) -> None:
        data = df[df[y] != 0]
        melted = data.melt(
                id_vars=['pointsAmount'],
                value_vars=['voronoiTime', 'delaunayTime'],
                value_name='Time',
                var_name="Algorithm"
        )

        melted['Algorithm'].replace(
            {
                'voronoiTime': "Voronoi",
                'delaunayTime': "Delaunay"
            }, 
            inplace=True
        )

        sns.lineplot(data=melted, x='pointsAmount', y='Time', hue='Algorithm', ax=ax)
        ax.grid(True)
    ...
    fig.tight_layout()
    fig.savefig("VoronoiMetrics.png")
