...
public:
  ...
  bool operator==(const Vector<D, S> &vec) const {
    for (size_t i = 0; i < D; i++) {
      if (m_data[i] != vec[i]) {
        return false;
      }
  ...
  Vector<D, S> operator+(Vector<D, S> vec) const {
    Vector<D, S> v(*this);
    v += vec;
    return v;
  }
...
std::ostream &operator<<(std::ostream &os, const Vector<D, S> &vec) {
  os << "(";
  for (size_t i = 0; i < D; ++i) {
    os << vec[i];
    if (i < D - 1) {
...
using Vec2u = Engine::Math::Vector<2, size_t>;
using Vec2 = Engine::Math::Vector<2, float>;
using Vec2i = Engine::Math::Vector<2, int32_t>;
using Color = Engine::Math::Vector<3, float>;
